...
filename_ins = importdata(fullfile(data_path, 'raw', 'raw_data_filenames.txt'));

% filename to be saved. 
% filename_out = 'C20110808_R03';
filename_outs = importdata(fullfile(data_path, 'raw', 'raw_data_filenames_abbre.txt'));

for f_id = 1:length(filename_ins)

    filename_in = filename_ins{f_id};
    filename_out = filename_outs{f_id};
    ...
    data = anp_data(dir_sec, filename_header, [0, 250]);
    ...
    call_anova_y(dir_sec, filename_out);
    
    %% log SNR: Compute log SNR
    ...
    %% log SNR: ANOVA
    dir_sec = [data_dir 'epoched_rsampsl_biprref_evkresp' ...
                z_dir '_' p_dir '_snrsurr/'];

    call_anova_y(dir_sec, filename_out);

    %% VELogP: calculate the vibration evoked log power
    data_sec = ['epoched_rsampsl_biprref_evkresp' z_dir '_' p_dir '/'];
    ...
    %% VELogP: Anova 
    dir_sec = [data_dir 'epoched_rsampsl_biprref_evkresp' ...
                z_dir '_' p_dir '_evkdpwr/'];

    call_anova_y(dir_sec, filename_out);
        
    %% HGP: find the total power in the HG band from VELogP
    data_sec = ['epoched_rsampsl_biprref_evkresp' z_dir '_' p_dir ...
                '_evkdpwr/'];
    ...
    %% HGP: anova
    dir_sec = ['epoched_rsampsl_biprref_evkresp' z_dir '_' p_dir ...
               '_evkdpwr_hgpcomp/'];
