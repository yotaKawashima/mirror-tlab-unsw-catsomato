function anp_figures_ph(data_dir, filename_header, options)
...
% Process log10 for p values, added by Phoebe
pvals = log10(pvals);
if isfield(options, 'clim') && ~isempty(options.clim)
    clim = [0, options.clim];
else
...
for k = 1:numel(options.foi)
    ...
    for l = 1:3
        figure(l); clf
        draw_biprref_ph(pvals(:, foi_ind, l), ch_label, metavars.custom.spatialconfig, clim)
