...
% Check what happens in time domain before bipolar re-referencing.
...
for i_session = 1:length(unique_sessionids)
    ...
    % Get unipolar channel from bipolar channel data.
    loadnames = dir(fullfile(data_dir, cat_name, ...
                    'epoched_rsampsl_biprref', '*.mat'));
    bp_data = load(fullfile(data_dir, cat_name, 'epoched_rsampsl_biprref', ...
              loadnames(1).name));
    ...
    if strcmp(bp_data.data.label{1}(1:3), 'raw')
        % the first bipolar channel
        bipolarchid = 1 + prod(bp_data.data.custom.spatialconfig);
        % # of bipolar channels
        nChan = bp_data.data.custom.nsignals - bipolarchid + 1; 
    else
        nChan = bp_data.data.custom.nsignals;
        bipolarchid = 1;
    end   
    labels = bp_data.data.label(bipolarchid:end);
    labels_invalidchs = labels(invalid_bpchs);
    invalid_unichs = [];
    for invalid_bp = 1:size(labels_invalidchs, 1)
        invalid_unichs = [invalid_unichs, ...
                          str2num(labels_invalidchs{invalid_bp}(10:12))];
        invalid_unichs = [invalid_unichs, ...
                          str2num(labels_invalidchs{invalid_bp}(16:18))];                      
    end
    
    invalid_unichs = unique(invalid_unichs);
    disp([cat_name, '-- unipolar channel id: ', num2str(invalid_unichs)]);
    ...
    for condid = 1:nCond
        ...
        for bipolar_channel = invalid_unichs
            % data.trial = channels x time x trials 
            % data_bipolar_now = freqs x trials
            data_unipolar_now = squeeze(data.trial(bipolar_channel, : ,:));
            
            % # of 0 across frequencies and trials for this channel.
            num_zero_vals = sum(sum(data_unipolar_now == 0));
            
            % # of trials x # of freqs
            num_vals = size(data_unipolar_now, 1) * size(data_unipolar_now, 2);
            
            if num_zero_vals ==  num_vals
                % Show nothing if all values are 0  
                %fprintf('All 0 val for %ich\n', bipolar_channel);
            elseif num_zero_vals  == 0
