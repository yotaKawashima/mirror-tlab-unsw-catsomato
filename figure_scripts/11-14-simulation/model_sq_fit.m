...
% First, find top 5% of channels that give the larger value of 
...
data_top5_struct = struct();
data_top5_struct(1).top_channels = top5_channels_S1;
data_top5_struct(2).top_channels = top5_channels_S2;
data_top5_struct(1).top_threshold = top5_threshold_S1;
data_top5_struct(2).top_threshold = top5_threshold_S2;
data_top5_struct(1).mean_logsnrs = top5_mean_logsnrs_S1;
data_top5_struct(2).mean_logsnrs = top5_mean_logsnrs_S2;
...
for area_id = 1:2
    ...
    logsnrs = data_top5_struct(area_id).mean_logsnrs;
    ...
    for bp_channel = 1:size(logsnrs, 1)
        ...
        for model_id = 1:4        
            switch model_id 
                ...
                case 4 % aSq(X) + bSq(Y) + cSq(X)Sq(Y) + dSq(XY) 
...
% Save data
save('Results_Sq_top5.mat', 'each_area');
