function [H, pValue, KSstatistic, XVal, YVal1, YVal2] = kstest2_y(x1, x2, varargin)
%KSTEST2 Two-sample Kolmogorov-Smirnov goodness-of-fit hypothesis test.
%   Copied from kstest2.m (9.7.0.1261785 (R2019b) Update 3') and add some 
%   lines to get x value giving KSstatistic. 
% 
...
%
%   [H,P,KSSTAT,XVal,YVal1,YVal2] = KSTEST2(...) also returns the XVal that
%   gives the K-S test statistic and corresponding x1 and x2 cdf value
%   (YVal1 and YVla2).
%
...
% Get index giving the max CDF 
%KSstatistic   =  max(deltaCDF);
[KSstatistic, x_ind]   =  max(deltaCDF);
XVal = binEdges(x_ind);
YVal1 = sampleCDF1(x_ind);
YVal2 = sampleCDF2(x_ind);
