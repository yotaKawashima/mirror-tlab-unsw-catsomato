...
for model_id = 1:4
    ...
    each_model_now = each_model_best(model_id);
    num_versions = size(each_model_now.error, 1);
    
    figure();
    scatter(1:num_versions, each_model_now.error, 100, 'filled');
    title(['model id ', num2str(model_id), ' across settings']);
    xlabel('Settings');
    ylabel('best error [dB]');
    
    minimum_errors_each(1, model_id) = min(each_model_now.error);
