...
% Here, we fit a model that has 3 parameters to actual data. 
...
model = @(a, b, c)model_all(a, b, c); % a,b, and c are inslide the Rect()
...
%% Rough grid search
[G_min_var1_1, G_min_var2_1, G_min_var3_1, G_error_1, G_errors_1] = ...
    fit_model_v1(model, a_range, b_range, c_range, mean_logsnr_1, sampling_rate);
...
% Check results 
model_best_1 = model(G_min_var1_1, G_min_var2_1, G_min_var3_1);
% Compute logSNR
...
title_text = ['Rect(X) + a*Rect(Y) + b*Rect(X)*Rect(Y) + c*Rect(XY)', ...
              ', a=', num2str(G_min_var1_1), ', b=', num2str(G_min_var2_1), ...
              ', c=', num2str(G_min_var3_1), ', error=', num2str(G_error_1)];
title(title_text);
...
varmax = 1;
tic;
[L_min_var1_1, L_min_var2_1, L_min_var3_1, L_error_1, L_errors_1, L_coordinate_1] = ...
...
% Check results 
model_best_1 = model(L_min_var1_1, L_min_var2_1, L_min_var3_1);
% Compute logSNR
[model_logsnrs, powers_tmp, freqs] = compute_logsnrs_y(model_best_1, sampling_rate);
...
title_text = ['Rect(X) + a*Rect(Y) + b*Rect(X)*Rect(Y) + c*Rect(XY)', ...
              ...
              ', c=', num2str(L_min_var3_1), ', error=', num2str(L_error_1)];
title(title_text);
%}
...
function [min_var1, min_var2, min_var3, error, errors] = fit_model_v1(model, var1range, var2range, var3range, datalogsnrs, sampling_rate)
...
%   
    errors = nan(length(var1range)*length(var2range)*length(var3range), 1);
    increment_ = 1;
    for var1_id = 1:length(var1range)
        ...
        for var2_id = 1:length(var2range)
            ...
            for var3_id = 1:length(var3range)
                ...
                %disp('-------')
                % Compute error 
                if var1 == 0 && var2 == 0 && var3 == 0 % If 0 amp signal
                    error_tmp = Inf;
                else 
                    error_tmp = compute_error(model(var1, var2, var3), ...
...
function [min_var1, min_var2, min_var3, error, errors, coordinate] = fit_model_v3(model, varmin, varmax, datalogsnrs, sampling_rate, iteration, tol)
...
function model = model_all(a, b, c)
    % frequency
    f1 = 23;
    f2 = 200;
    ...
    % model 
    model = Rect_comp1 + a*Rect_comp2 + b*Rect_comp1 .* Rect_comp2 ...
            + c*Rect_mult_sines;
end