...
iterations = [2, 3, 3, 4] * 10; % iterate 10 times for each coefficient
varmin = -1;
varmax = 1;
...
initial_values = [-1, -0.5, 0, 0.5, 1];

for model_id = 1:4   
    for initial_value = initial_values
        switch model_id 
            case 1 % aRect(X) + bRect(Y)
                iteration = iterations(model_id);
                num_order = 2;
                L_error_matrix = ones(num_order*length(initial_values), 1);
                best_param_matrix = ones(num_order*length(initial_values), 2);
                model = @(a, b)model_plus(a, b);
            ...
            case 2 % aRect(X) + bRect(Y) + cRect(X)Rect(Y)
                ...
                num_order = 6;
                L_error_matrix = ones(num_order*length(initial_values), 1);
                best_param_matrix = ones(num_order*length(initial_values), 3);
                model = @(a, b, c)model_without_RXY(a, b, c); 
            ...
            case 4 % aRect(X) + bRect(Y) + cRect(X)Rect(Y) + dRect(XY) 
                ...
                num_order = 24;            
                L_error_matrix = ones(num_order*length(initial_values), 1);            
                best_param_matrix = ones(num_order*length(initial_values), 4);
                model = @(a, b, c, d)model_all(a, b, c, d); 
...
for model_id = 1:4
    ...
    num_versions_s = size(each_model_now_s.error, 1);
    
    % Combine two 
    num_versions_all = num_versions + num_versions_s;
    error_all_models = [each_model_now.error; each_model_now_s.error];
    model_logsnr_all_models = [each_model_now.model_logsnr; ...
                               each_model_now_s.model_logsnr];
                           
    figure();
    scatter(1:num_versions_all, error_all_models, 100, 'filled');
    title(['model id ', num2str(model_id), ' across settings']);
    xlabel('Settings');
    ylabel('best error [dB]');
    ylim([70, 170]);
    
    minimum_errors_each(1, model_id) = min(error_all_models);
    
end % model_id = 1:4

% Reorder
figure();
scatter(1:4, minimum_errors_each, 100, 'filled');
title('Best for each model');
xlabel('model id');
ylabel('min across settings');
ylim([70, 170]);
...
%% Functions
function [min_var1, min_var2, error, errors, coordinate] = fit_model_param2(model, varmin, varmax, datalogsnrs, sampling_rate, iteration, initial_value, order)
...
function [min_var1, min_var2, min_var3, error, errors, coordinate] = fit_model_param3(model, varmin, varmax, datalogsnrs, sampling_rate, iteration, initial_value, order)
% Find var1 and var 2 giving the local minimum by line serach. We
% minimise error (difference of logSNR between model and data across foi).
%   Input:  func = function to be accessed. Consider two-variable
%                  function (Model)
...
function [min_var1, min_var2, min_var3, min_var4, error, errors, coordinate] = fit_model_param4(model, varmin, varmax, datalogsnrs, sampling_rate, iteration, initial_value, order)
% Find var1 and var 2 giving the local minimum by line serach. We
% minimise error (difference of logSNR between model and data across foi).
%   Input:  func = function to be accessed. Consider two-variable
%                  function (Model)
