...
bonferroni_correction = 8; % number of models we tested.
...
%% HSq compare S1 and S2 for each model
% Get error from each area
model_orders = [1, 3, 2, 4];
error_each_area = struct();
disp('HSq');
...
for area_id = 1:2
    ...
    for order_id = 1:4
        ...
        error_now = errors(:,model_id);
        % Remove the worse 1% channels for a display purpose for
        % HSq(X)+HSq(Y) and HSq(X)+HSq(Y)+HSq(XY)
        if (area_id == 1 && model_id == 1) || ...
           (area_id == 1 && model_id == 3)
            error_threshold = prctile(error_now, 99);
            error_temp = error_now;
        ...
        switch model_id
            case 1
                title_now = 'HSq(X)+HSq(Y)';
            case 2
                title_now = 'HSq(X)+HSq(Y)+HSq(X)HSq(Y)';
            case 3
                title_now = 'HSq(X)+HSq(Y)+HSq(XY)';
            case 4
                title_now = 'HSq(X)+HSq(Y)+HSq(XY)+HSq(X)HSq(Y)';
        end % swithch model_id   
...
%% HSq compare models for each area
% Plot culmtive probability distribution
colours = [[0, 0.4470, 0.7410];...
           [0.4940, 0.1840, 0.5560];...
           [0.8500, 0.3250, 0.0980];...
