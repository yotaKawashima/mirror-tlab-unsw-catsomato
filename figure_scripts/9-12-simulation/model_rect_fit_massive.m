function model_rect_fit_massive(channel_id, area_id)
%%Fit all Rect models to a channel data. 
% copied from (test_modeling_with_other_parms_v4.m)
...
varmin = -1;
varmax = 1;
iterations = [30, 45, 45, 60]; % iterate 5 times for each coefficient
...
for model_id = 1:4
    ...
    for init_ind = 1:length(initial_values)
        initial_value = initial_values(init_ind);
        switch model_id 
            case 1 % aRect(X) + bRect(Y)
                ...
                for order_id = 1:num_order
                    ...
                    if init_ind == 1 && order_id == 1
                        L_error_matrix = nan(num_order*length(initial_values), 1);
                        best_param_matrix = nan(num_order*length(initial_values), 2);
                        model_signal_matrix = nan(num_order*length(initial_values), ...
                                                    length(model_signal));
            ...
            case 2 % aRect(X) + bRect(Y) + cRect(X)Rect(Y)
                ...
                for order_id = 1:num_order
                    ...
                    if init_ind == 1 && order_id == 1
                        L_error_matrix = nan(num_order*length(initial_values), 1);
                        best_param_matrix = nan(num_order*length(initial_values), 3);                        
                        model_signal_matrix = nan(num_order*length(initial_values), ...
                                                    length(model_signal));
                    end % init_ind == 1 && order_id == 1      
                    id_incre = id_incre + 1;
            ...
            case 4 % aRect(X) + bRect(Y) + cRect(X)Rect(Y) + dRect(XY) 
                ...
                for order_id = 1:num_order
                    ...
                    if init_ind == 1 && order_id == 1
                        L_error_matrix = nan(num_order*length(initial_values), 1);
                        best_param_matrix = nan(num_order*length(initial_values), 4);                        
                        model_signal_matrix = nan(num_order*length(initial_values), ...
                                                    length(model_signal));
                    end % init_ind == 1 && order_id == 1
                    id_incre = id_incre + 1;
    ...
    for i_iter = 1:iteration
        ...
        if mod(i_iter, 2) == 1 % Fit var1 first
            ...
            if i_iter == 1
                ...
                varmin_now = varmin + initial_value;
                varmax_now = varmax + initial_value;
            else
