function model_hsq_fit_massive_v2_each(channel_id, area_id, model_id)
%%Fit each Half squaring models to a channel data. 
...
for init_ind = 1:size(initial_values_matrix, 1)
    ...
    switch model_id 
        ...
        case 2 % aHSq(X) + bHSq(Y) + cHSq(X)HSq(Y)
            order_matrix = perms([3, 2, 1]);
            num_order = size(order_matrix, 1);
            model = @(a, b, c)model_without_HSqXY(a, b, c); 
        ...
        case 3 % aHSq(X) + bHSq(Y) + cHSq(XY)
            order_matrix = perms([3, 2, 1]);
            num_order = size(order_matrix, 1);
            model = @(a, b, c)model_without_HSqXHSqY(a, b, c);
        ...
        case 4 % aHSq(X) + bHSq(Y) + cHSq(X)HSq(Y) + dHSq(XY) 
            order_matrix = perms([4, 3, 2, 1]);
            num_order = size(order_matrix, 1);
            ...
            for order_id = 1:num_order
                ...
                % Line search
                [L_min_var1, L_min_var2, L_min_var3, L_min_var4, L_error, ~, ~] = ...
                    fit_model_param4(model, varmin, varmax, observed_logsnr, ...
                    sampling_rate, iteration, initial_values, order);
                best_parameters = [L_min_var1, L_min_var2, ...
                ...
                if init_ind == 1 && order_id == 1
                    L_error_matrix = nan(num_order*size(initial_values_matrix, 1), 1);
                    best_param_matrix = nan(num_order*size(initial_values_matrix, 1), 4);                        
                    model_signal_matrix = nan(num_order*size(initial_values_matrix, 1), ...
                                                length(model_signal));
                end % init_ind == 1 && order_id == 1
                id_incre = id_incre + 1;
...
% Save data 
% all settings 
dir_name_all_settings = './hsq_top10_results_all_settings_v2_each/';
...
dir_name_best_setting = './hsq_top10_results_best_setting_v2_each/';
...
function [min_var1, min_var2, min_var3, min_var4, err, errors, coordinate] = fit_model_param4(model, varmin, varmax, datalogsnrs, sampling_rate, iteration, initial_values, order)
% Find var1 and var 2 giving the local minimum by line serach. We
% minimise error (difference of logSNR between model and data across foi).
%   Input:  func = function to be accessed. Consider two-variable
%                  function (Model)
            ...
            if i_iter == 1
                switch target 
                    ...
                    case 3
                        ...
                        varmax_now = varmax + initial_values(3);
                    case 4
                        varfixed = initial_values(1:3);
                        varmin_now = varmin + initial_values(4);
                        varmax_now = varmax + initial_values(4);
                end % switch target                       
            else
                switch target
                    case 1
                    ...
                    case 4
                        ...
                        varmin_now = coordinate(i_iter - 1, 4);
                        varmax_now = varmax + coordinate(i_iter - 1, 4);                                        
                end % switch target                            
            end % i_iter == 1
            [new_var, err]= golden_section_parm4(model, datalogsnrs, ... 
                sampling_rate, varmin_now, varmax_now, varfixed, target, tol);
